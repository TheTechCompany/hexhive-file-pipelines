...
channel = None
credentals = pika.PlainCredentials(os.getenv('MQ_USER') or 'rabbitmq', os.getenv('MQ_PASS') or 'rabbitmq')
parameters = pika.ConnectionParameters(os.getenv('MQ_URL'), 5672, '/', credentals)
...
def handle_delivery(channel, method, header, body):
    ...
    try:
        input_json = json.loads(input_ref)
    except json.JSONDecodeError:
        print("No json in message")
