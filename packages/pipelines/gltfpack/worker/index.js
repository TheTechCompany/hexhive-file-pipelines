...
channel.consume(inQueue, async (msg) => {
  try{
    ...
    await IPFS.getFile(json.input_cid, '/tmp/' + json.input_cid + '.glb')


    //Run processor
    process.chdir('/tmp')
    exec(`/usr/local/bin/gltfpack -i ./${json.input_cid}.glb -tc -kn -km -c -o ./${json.job_id}.glb`, async (err, stdout, stderr) => {
      if(err) {
        channel.nack(msg)
